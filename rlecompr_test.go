...
func TestRLEEncode(t *testing.T) {
	for _, tt := range rleEncodeTests {
		got, ok := RLEEncode(tt.in)
		if got != tt.want || ok != tt.ok {
			t.Errorf("RLEEncode(%s) = %s, %t; want %s, %t", tt.in, got, ok, tt.want, tt.ok)
		}
	}
}
...
func TestRLEDecode(t *testing.T) {
	for _, tt := range rleDecodeTests {
		got, ok := RLEDecode(tt.in)
		if got != tt.want || ok != tt.ok {
			t.Errorf("RLEDecode(%s) = %s, %t; want %s, %t", tt.in, got, ok, tt.want, tt.ok)
		}
	}
}
